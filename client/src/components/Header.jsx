...
export default function Header() {
  ...
  const navLinkClass = (path) =>
    `group relative overflow-hidden rounded-xl px-4 py-3 text-sm font-semibold transition-all duration-300 ease-out flex items-center gap-3 ${pathname === path
  ...
  return (
    <SidebarContext.Provider value={{ isSidebarOpen }}>
      ...
      {/* Modern Floating Hamburger Button - Hidden on Post Pages */}
      {!pathname.startsWith('/post/') && (
        <div className={`fixed top-6 left-6 z-[100] transition-all duration-500 ease-out ${isSidebarOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          <button
      ...
      <div className={`fixed inset-y-0 left-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-r border-gray-200/50 dark:border-gray-700/50 shadow-2xl z-[90] transition-all duration-500 ease-out ${isSidebarOpen ? 'w-80' : '-translate-x-full'}`}>
      ...
      {isSidebarOpen && (
        <div
          className="fixed inset-0 bg-black/20 backdrop-blur-sm z-[80] transition-all duration-500 ease-out"
